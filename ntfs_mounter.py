...
class NTFSMounter(QMainWindow):
    ...
    def initUI(self):
        ...
        self.disk_identifier_input.setPlaceholderText('在这里输入你想要挂载的磁盘标识符(IDENTIFIER)')
    ...
    def mountNTFSDisk(self):
        ...
        if not disk_identifier:
            self.status_label.setText('请刷新磁盘列表，并输入想要挂载的磁盘标识符')
            return
        
        reply = QMessageBox.question(self, '确认', f'确定要挂载磁盘 {disk_identifier} 吗?', QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
        # 尝试挂载磁盘
        if reply == QMessageBox.Yes:
            try:
                # 方案一：确认挂载后需要在终端输入计算机密码。
                QMessageBox.information(self, '注意', '请在终端输入计算机密码。')
